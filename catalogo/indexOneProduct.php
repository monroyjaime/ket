...
$backCond =   '<a href="#" onClick="backCatalogo('.$dptoId.')" title="Pag. Prev."><i class="bi bi-arrow-left-circle-fill icon-dark-blue icon-large"></i></a>';

$productId = (isset($_GET['prod_id']))?  intval($_GET['prod_id']) : 1;
...
$query  = "SELECT id,code,name,photo_url,".$strTipoPrecio." AS precio,unit,current_stock FROM productos WHERE id=".$productId;
...
foreach ($consult1 as $value1){
    ...
    $productVal_cost = floatval($value1->precio);
    $productVal_cost_80 = floatval($value1->precio)*.8;
    ...
    if($role>-1)
    {
        $tags .=            '<h5 class="card-text" >'.$labelTipoPrecio.': $'.number_format($productVal_cost,3,",").'</h5>';
...
<html>
   ...
   <script>
        function backCatalogo(dpto)
        {
            urlString =  "indexDptoAll2.php?dpto_id="+dpto;
