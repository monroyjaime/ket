...
$numRows = (isset($_GET['rows']))? intval($_GET['rows']) : 4;
$db = new DB();
...
if($catId>0)
{
 ...
 //   $tags .= '<div class="row align-items-start row-md">';
    ...
    $tags .=    '<h2>Catalogo de '.$arrCategories[$catId-1]->catName;
    
    $numProducts=0;
    $query  = "SELECT id,code,descripcion,photo_url FROM hist_gal WHERE show='t' AND category=";
    $query .= $arrCategories[$catId-1]->id." AND photo_url != 'empty.jpg' ORDER BY id";
    ...
    $numPages = (($numProducts % (4*$numRows)) == 0 )? intval($numProducts / (4*$numRows)) : (intval($numProducts / (4*$numRows))) + 1 ;
    $tags .= ' ('.$pageNum.' / '.$numPages.')</h2>';
    $tags .= '</div>';
