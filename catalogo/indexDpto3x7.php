...
$tipo_precio = isset($_GET['tipo_precio']) ? $_GET['tipo_precio'] : 'sin';

// Determinar qué campo de precio usar
if ($tipo_precio == 'mayorista') {
    $campo_precio = 'cost_mayor';
    $label_precio = 'Precio Mayorista';
} elseif ($tipo_precio == 'minorista') {
    $campo_precio = 'cost_max';
    $label_precio = 'Precio';
} else {
    $campo_precio = null;
    $label_precio = null;
...
if ($campo_precio) {
    $query = "SELECT code, name, photo_url, $campo_precio AS precio FROM productos ";
    $query .= "WHERE show='t' AND dpto_id=".$dptoId." AND photo_url != 'empty.jpg' AND $campo_precio > 0 ";
    $query .= "ORDER BY orden, code ";
    $query .= "LIMIT ".$productosPorPagina." OFFSET ".$offset;
} else {
    $query = "SELECT code, name, photo_url FROM productos ";
...
if ($numProducts > 0) {
    ...
    foreach ($consult1 as $producto) {
        ...
        // Mostrar precio si corresponde
        if ($campo_precio && isset($producto->precio)) {
            $precio = floatval($producto->precio);
            $tags .= '<div class="card-footer text-center" style="background-color: #f0f0f0; padding: 4px; font-size: 8pt;">';
            $tags .= '<strong>' . $label_precio . ': $' . number_format($precio, 3, ',', '.') . '</strong>';
            $tags .= '</div>';
        }
