...
$role = 1; //(isset($_SESSION['role']))? intval($_SESSION['role']) : -1;
...
$query .= " FROM pedido_carrito a, productos b WHERE a.user_num=".$numUsr." AND b.code=a.product_code";
$consult = $db->consultas($query);
foreach ($consult as $value){
    $objRtn = new stdClass();
    $tipoPrecio = $value->tipo_precio;
    ...
    $objRtn->precio =$currPrecio;
    $objRtn->check_prec = $tipoPrecio;
    $objRtn->monto =number_format (floatval($currPrecio)*intval($value->cantidad),3);
    $listaProd[] = $objRtn;
    $numProd++;
}
