<?php
// update_productos-upd-cvs.php - Con actualización automática de PDFs
...
// ============================================
// ARRAY PARA REGISTRAR DEPARTAMENTOS AFECTADOS
// ============================================
$dptos_afectados = [];

function registrar_dpto_afectado($dpto_id, &$dptos_afectados) {
    if ($dpto_id > 0 && !in_array($dpto_id, $dptos_afectados)) {
        $dptos_afectados[] = $dpto_id;
        log_update("📌 Departamento $dpto_id registrado para actualización de PDFs");
    }
}

function actualizar_pdfs_dptos($dptos_afectados, $db) {
    if (empty($dptos_afectados)) {
        log_update("📄 No hay departamentos para actualizar PDFs");
        return 0;
    }
    
    log_update("🖨️ Iniciando actualización de PDFs para departamentos: " . implode(', ', $dptos_afectados));
    ...
    foreach ($dptos_afectados as $dpto_id) {
        ...
        foreach ($tipos as $tipo) {
            ...
            if ($return_code !== 0) {
                $dpto_exitoso = false;
                log_update("❌ Error generando PDF para dpto $dpto_id tipo $tipo: " . implode("\n", $output));
            }
        }
        
        if ($dpto_exitoso) {
            $exitosos++;
            log_update("✅ PDFs generados correctamente para departamento $dpto_id");
            
            // Actualizar fecha de actualización en la BD
            $fecha_actual = date('Y-m-d H:i:s');
            $db->querySet("UPDATE departamentos SET pdf_last_upd = '$fecha_actual' WHERE id = $dpto_id");
        }
    }
    
    log_update("✅ Actualización de PDFs completada: $exitosos de " . count($dptos_afectados) . " departamentos");
    return $exitosos;
}

// Función para obtener el departamento de un producto por su código
function obtener_dpto_id_por_codigo($db, $codigo) {
    $result = $db->consultas("SELECT dpto_id FROM productos WHERE code = '$codigo'");
    if (!empty($result)) {
        return $result[0]->dpto_id;
    }
    return 0;
}

// Configuración de archivo de precios
...
function reportPrecioChange($tipo, $code, $prevPrecio, $newPrecio, $prevCosto = '---', $newCosto = '---') {
    ...
    if (function_exists('agregarLineaCSV')) {
        if (agregarLineaCSV($fila, $filePrecio) === false) {
            ...
            log_update("📝 preciosChanged.csv actualizado: $code ($tipo) $prevPrecio -> $newPrecio");
        }
    }
}
...
foreach ($consult1 as $index => $value1) {
    ...
    foreach($consult2 as $value2) {
        $found = 1;
        $dpto_id_actual = obtener_dpto_id_por_codigo($db, $value1->code);
        
        // 1. Actualizar nombre/descripcion
        if($value1->name != $value2->name) {
            if($db->querySet("UPDATE productos SET name = '".$value1->name."' WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['descripcion']."::updated description:: CODE: ". $value1->code);
                registrar_dpto_afectado($dpto_id_actual, $dptos_afectados);
        ...
        if(number_format(floatval($value1->cost_max),3) != number_format(floatval($value2->cost_max),3)) {
            if($db->querySet("UPDATE productos SET cost_max = ". $value1->cost_max." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['cost_max']."::updated cost_max:: CODE: ". $value1->code);
        ...
        if($value1->unit != $value2->unit) {
            if($db->querySet("UPDATE productos SET unit = '".$value1->unit."' WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['unit']."::updated unit:: CODE: ". $value1->code);
                registrar_dpto_afectado($dpto_id_actual, $dptos_afectados);
                $counters['unit']++;
            }
        }
        ...
        if($value1->current_stock != $value2->current_stock) {
            if($db->querySet("UPDATE productos SET current_stock = ". $value1->current_stock." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['current_stock']."::updated Stock:: CODE: ". $value1->code);
        ...
        if($value1->dpto_code != $value2->dpto_code) {
            ...
            // Registrar departamento ANTIGUO
            registrar_dpto_afectado($dpto_id_actual, $dptos_afectados);
            // Registrar departamento NUEVO
            registrar_dpto_afectado($newDptoId, $dptos_afectados);
            
            if($db->querySet("UPDATE productos SET dpto_code ='".$value1->dpto_code."', dpto_id=".$newDptoId." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['dpto_code']."::updated dpto_code:: CODE: ". $value1->code);
                $counters['dpto_code']++;
            }
        }
        
        // 6. Actualizar cost_oferta
        if(number_format(floatval($value1->cost_oferta),3) != number_format(floatval($value2->cost_oferta),3)) {
            if($db->querySet("UPDATE productos SET cost_oferta = ". $value1->cost_oferta." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['cost_oferta']."::updated cost_oferta:: CODE: ". $value1->code);
                $counters['cost_oferta']++;
            }
        }
        
        // 7. Actualizar cost_mayor
        if(number_format(floatval($value1->cost_mayor),3) != number_format(floatval($value2->cost_mayor),3)) {
            if($db->querySet("UPDATE productos SET cost_mayor = ". $value1->cost_mayor." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['cost_mayor']."::updated cost_mayor:: CODE: ". $value1->code);
        ...
        if($value1->stock_lleg != $value2->stock_lleg) {
            if($db->querySet("UPDATE productos SET stock_lleg = ". $value1->stock_lleg." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['stock_lleg']."::updated Stock_llegando:: CODE: ". $value1->code);
                $counters['stock_lleg']++;
            }
        }
        
        // 10. Actualizar relacionado
        if($value1->relacionado != $value2->relacionado) {
            if($db->querySet("UPDATE productos SET relacionado = '".$value1->relacionado."' WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['relacionado']."::updated relacionado:: CODE: ". $value1->code);
        ...
        if(number_format(floatval($value1->costo),3) != number_format(floatval($value2->costo),3)) {
            if($db->querySet("UPDATE productos SET costo = ". $value1->costo." WHERE code ='".$value1->code."'") == 1) {
                log_update($counters['costo']."::updated costo:: CODE: ". $value1->code);
    ...
    if($found == 0) {
        ...
        if($db->querySet($queryInsert) == 1) {
            ...
            registrar_dpto_afectado($dptoId, $dptos_afectados);
            $counters['nuevos']++;
        } else {
...
$consult4 = $db->consultas("SELECT code, dpto_id FROM productos WHERE code NOT IN (SELECT code FROM prod_name) ORDER BY code");

foreach($consult4 as $val) {
    $dpto_id_eliminado = $val->dpto_id;
    $queryDel = "DELETE FROM productos WHERE code='".$val->code."'";
    if($db->querySet($queryDel) == 1) {
        log_update("✅ ELIMINADO: " . $val->code);
        registrar_dpto_afectado($dpto_id_eliminado, $dptos_afectados);
        $counters['eliminados']++;
    } else {
...
// ============================================
// ACTUALIZAR PDFs DE DEPARTAMENTOS AFECTADOS
// ============================================
if (!empty($dptos_afectados)) {
    log_update("📄 Departamentos afectados por cambios: " . implode(', ', $dptos_afectados));
    $pdfs_actualizados = actualizar_pdfs_dptos($dptos_afectados, $db);
    log_update("🖨️ PDFs actualizados para $pdfs_actualizados departamentos");
} else {
    log_update("📄 No se detectaron cambios que requieran actualización de PDFs");
}
...
$resumen = [
    ...
    "📄 Departamentos con PDFs actualizados: " . count($dptos_afectados),
