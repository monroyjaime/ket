...
*/
        ...
        function getSelected(){
          ...
          $.each(selectedRows, function(index, value) {
            ...
            tags += '<tr>';
            tags +=   '<td id = "Status"></td>'
            tags +=   '<td id = "Code">'+value.code+'</td>';
            arrCode.push(value.code);
            tags +=   '<td>'+value.name+'</td>';
            tags +=   '<td><input id = "Cantidad" type="number" min="0" class="form-control" value="1" oninput="catidadCambia()" disabled/></td>';
            tags +=   '<td>'+value.unit+'</td>';
            tags +=   '<td>$'+value.cost_max+'</td>';
            tags +=   '<td><input id = "Monto" type="text" class="form-control-plaintext text-end" value="$ '+value.cost_max+'" readonly/></td>';
          ...
          tags += '<button type="button" class="btn btn-primary btn-sm"  onClick="registrarPedido()" style="margin: 1px 40px 1px;"><i class="bi bi-cart-check"></i> Registrar Pedido</button> '
          strCodes = arrCode.join(',');
          //console.log("strCodes:"+strCodes);
          if(arrCode.length > 0)
            $.post("../php/insertDeleteCarrito.php",{strCodes: strCodes}, 
              function(data,status){
                  console.log('message recibed from Srv: '+data);
              });
        ...
        function registrarPedido(){
          ...
          for (let i = 1; i < rows.length; i++) {
                ...
                codes.push(rows[i].cells[1].innerHTML)
                precios.push(rows[i].cells[5].innerHTML)
                //console.log('row'+(i)+' cell 1:'+rows[i].cells[1].innerHTML);
                //console.log('row'+(i)+' cell 5:'+rows[i].cells[5].innerHTML);
          ...
          $.each($('#ModalMakePedido #Cantidad'),function(index,valor){
            ...
            producto.precio = precios[index].replace(',', '.').replace('$','');
            productos.push(producto)
            console.log('cantidad: '+valor.value+' code:'+codes[index]+' Precio:')
          });

          var paramJSON = JSON.stringify(productos);
          ...
          $.post("../php/insertPedidoGeneral.php",
            {data: paramJSON}, 
            function(data,status){
                console.log('data recibed in Srv: '+data);
            });
        ...
        function catidadCambia(){
          ...
          montos=[];
          for (let i = 1; i < rows.length; i++) {
                precios.push(parseFloat(rows[i].cells[5].innerHTML.replace(',', '.').replace('$','')))
                montos.push(parseFloat(rows[i].cells[6].innerHTML.replace(',', '.').replace('$','')))
              }
          $.each($('#ModalMakePedido #Cantidad'),function(index,valor){
        ...
        function showPedidoClient(){
          $.post("../php/getListPedidoClient.php",
            {}, 
            function(data,status){
                console.log('data recibed in Srv: '+data);
                //tags = data;
